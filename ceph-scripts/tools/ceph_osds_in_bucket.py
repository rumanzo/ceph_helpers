...
def deviceclass(devclass):
    global nodes_by_id
    tree = commands.getoutput('ceph osd tree -f json')
    all_nodes = json.loads(tree)['nodes']
    nodes_by_id, nodes_by_name = prepare(all_nodes)
    try:
        devices =  [x[0] for x in nodes_by_name.items() if 'device_class' in x[1] and devclass in x[1]['device_class']]
    except KeyError:
        raise Exception("Unknown CRUSH device class '%s'" % devclass)
    return devices
